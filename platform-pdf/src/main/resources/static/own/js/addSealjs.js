...
$(document).ready(function () {
    ...
    $(".pic").droppable({      //拖动的目标接收操作
        ...
        drop: function (event, ui) {
            ...
            $.getScript("/essplatformpdf/own/js/addSealjs2.js"); //添加新的js实现对加入页面的相关操作
       }
    });
    setSize(); //设置pdf显示的大小
...
function add2file() {
    ...
    for (var i = 1; i <= maxPage; i++) {
        ...
        for (var j = 1; j <= $("#" + i).find(".ele").length; j++) {
            ...
            y = y.toFixed(1);
            //生成数组
            var s={"page": i, "sealId": sealId, "x": x, "y": y, "Type": Type};
            arrays.push(s);
        }
    }
    var jsonString = JSON.stringify(arrays);
    $.ajax({
        url:"/essplatformpdf/sign",
...
function insert() {
        ...
        if (num < maxPage + 1 && num > 0) {
            ...
            $.getScript("/essplatformpdf/own/js/addSealjs2.js");//加载签章相关的js
